...
class PlanetScene: SCNScene //, SCNSceneRendererDelegate
{
    override init() {
        let planetSphere = SCNSphere.init(radius: 10)
        let planetScene = SCNScene.init()
        let planetMap = PlanetMap(size:CGSize(width: 1000, height: 1000))
        ...
        cameraNode.position = SCNVector3(planetNode.position.x,planetNode.position.y,planetNode.position.z+19)
        
        super.init()
        
        rootNode.addChildNode(cameraNode)
        rootNode.addChildNode(planetNode)
        self.background.contents = UIColor.clear
